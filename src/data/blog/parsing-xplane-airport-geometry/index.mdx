...
The left image connects node coordinates with straight lines. The right is what the taxiway looks like.
...
function sampleQuadraticBezier(p0: Point, p1: Point, p2: Point, steps = 60): Point[] {
  const points: Point[] = [];
...
5. Control points can be anywhere. Close to the node means a gentle curve. Far from the node means a sharp bend. At the same position as the node is a plain node.
