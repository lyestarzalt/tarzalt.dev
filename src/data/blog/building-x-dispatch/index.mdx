...
![X-Dispatch world map with airports](/images/projects/x-dispatch/x-dispatch-map.png)

MapLibre GL JS ticked every box: polygons, GeoJSON shapes, extensible, stable, and it handles coordinate projections for me. That last part was the big one. All the projection math I was doing by hand before? MapLibre just does it. I could even add 3D terrain if I wanted. Basically I could do anything, limited only by the user's machine.
...
I wrote the full technical breakdown. Then I tackled linear features, the painted lines and taxiway lights.
...
One cool thing about X-Plane: they open sourced their airport data through the [Scenery Gateway](https://gateway.x-plane.com/). People design airports, upload them, and if approved, they appear in the next sim update. This was one of the smartest things Laminar ever did. Thousands of airports in X-Plane are handcrafted by the community. People spending hours looking at real charts, overlaying Google Maps imagery, getting taxiway widths and gate positions right so that a default airport actually looks like the real thing. It's a massive, quiet effort that most users never think about.

That's part of why I wanted to build this. I wanted to give those airports justice. Let people actually see the detail that went into them instead of picking an ICAO code from a list and hoping for the best. I spent a good chunk of one summer working on all the Algerian airports myself. Algiers, Oran, Constantine, the smaller ones. I had a blast doing it. So I know what it takes. When someone's airport renders beautifully in X-Dispatch, with every taxiway and gate in the right place, that's their work being shown off.
...
![Flight setup in X-Dispatch: pick your aircraft, gate, weather, and launch](/images/projects/x-dispatch/x-dispatch-flight-setup.png)

Looked good. But then I sat there thinking, right, they picked a gate... now what? How does X-Plane actually know about this?
...
![3D terrain view in X-Dispatch](/images/projects/x-dispatch/x-dispatch-terrain.png)
...
![Weather overlay in X-Dispatch](/images/projects/x-dispatch/x-dispatch-weather.png)

![SID/STAR procedure visualization](/images/projects/x-dispatch/x-dispatch-procedures.png)
...
![Addon manager in X-Dispatch](/images/projects/x-dispatch/x-dispatch-addon1.png)
