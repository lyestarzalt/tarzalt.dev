...
The bezier curve system is identical. Same node codes, same control point mirroring. If you can parse pavements, you already have most of the work done. See [parsing apt.dat geometry]({{< relref "/posts/parsing-xplane-airport-geometry" >}}) for the full breakdown of bezier math and the four connection types.
...
The bezier rules are the same as for pavements. Straight lines between 111 nodes, quadratic beziers when one node is a 112, cubic beziers when both are 112. See [the four connection types]({{< relref "/posts/parsing-xplane-airport-geometry#the-four-connection-types" >}}) for the geometry.
...
For the geometry explanation, see the [split bezier section]({{< relref "/posts/parsing-xplane-airport-geometry#sharp-corners--split-beziers" >}}) in the previous post. The consequence for linear features is worse than a visual spike. You lose type information.
...
- [Parsing X-Plane airport geometry from apt.dat]({{< relref "/posts/parsing-xplane-airport-geometry" >}}) — Bezier math, control point mirroring, winding order
